...
export default function GenealogyTab({ animal }) {
  ...
  const NodeContent = ({ animal, label, variant }) => (
    <div className="flex flex-col items-center z-10 shrink-0">
      <div className={`rounded-full border-2 overflow-hidden bg-white transition-all duration-200 ${
        variant === "main" 
          ? 'w-20 h-20 sm:w-24 sm:h-24 border-[#1B4820] ring-4 ring-emerald-100/80 shadow-lg scale-105' 
          : 'w-14 h-14 sm:w-16 sm:h-16 border-neutral-200/80 shadow-sm group-hover:scale-105 group-hover:border-[#1B4820]/40 group-hover:shadow-md'
      }`}>
      ...
      <div className="mt-1.5 text-center max-w-[85px] sm:max-w-[110px] flex flex-col items-center">
        <p className={`font-black leading-tight ${variant === "main" ? 'text-[#1B4820] text-sm sm:text-base' : 'text-neutral-800 text-xs sm:text-sm group-hover:text-[#1B4820] transition-colors'}`}>
          #{animal?.number || '---'}
        </p>
